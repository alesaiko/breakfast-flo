 ...
 */
#define AVTAB_XPERMS_OPTYPE	BIT(2)
...
static unsigned int __read_mostly avtab_android_m_compat;

static inline void avtab_android_m_compat_set(void)
{
	if (likely(avtab_android_m_compat))
		return;
...
int avtab_write_item(struct policydb *p, struct avtab_node *cur, void *fp)
{
	...
	if (avtab_android_m_compat && (cur->key.specified & AVTAB_XPERMS) &&
		...
		buf16[3] = cpu_to_le16(avtab_xperms_to_optype(
				       cur->key.specified));
	else
