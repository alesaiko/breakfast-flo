...
{
	/* Return early if input device does not exist */
	if (IS_ERR_OR_NULL(data->idev))
		return;
...
lid_create_device(const char *name, const char *phys)
{
	struct input_dev *idev;
	int ret;
...
static int __devinit lid_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(data)) {
		dev_err(&pdev->dev,
			"Unable to allocate memory for LID device data\n");
		return -ENOMEM;
	}
	...
	data->idev = lid_create_device(LID_NAME, LID_PHYS);
	if (IS_ERR_OR_NULL(data->idev)) {
		dev_err(&pdev->dev, "Unable to allocate LID input device\n");
		ret = -ENOMEM;
		goto fail_create;
	}
	...
	if (IS_ERR_VALUE(ret)) {
		dev_err(&pdev->dev,
			"Unable to request a GPIO number %u\n", data->gpio);
		goto fail_gpio;
	}
	...
	if (IS_ERR_VALUE(ret)) {
		dev_err(&pdev->dev,
			"Unable to request an IRQ for the LID device\n");
		goto fail_irq;
	}
